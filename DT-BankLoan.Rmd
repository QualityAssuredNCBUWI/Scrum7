...
set.seed(13)
newDataset_lead_1 <-sample.split(Y=data_lead_1$lead, SplitRatio = 0.30)
newDataset_lead_0 <-sample.split(Y=data_lead_0$lead, SplitRatio = 0.65)

trainData <-rbind(data_lead_1[newDataset_lead_1,], data_lead_0[newDataset_lead_0,])
testData <- rbind(data_lead_1[!newDataset_lead_1,][1:130,], data_lead_0[!newDataset_lead_0,])
...
DTmodel <- rpart(lead ~ education + age + job + marital + deposit + balance + loan + housing, method="class", data=trainData, parms = list (split ="information gain"), control = rpart.control(minsplit = 10, maxdepth = 3))  
```
